...
const _graveKeywords = <String>[
  'chatgpt', 'copilot', 'claude', 'gemini', 'perplexity', 'bard', 'bing chat',
  'character.ai', 'pi chat', 'mistral', 'deepseek',
  'telegram', 'whatsapp', 'discord', 'signal', 'messenger',
  'anydesk', 'teamviewer', 'rustdesk', 'parsec', 'chrome remote', 'rdp', 'vnc',
];
...
const _mediaKeywords = <String>[
  ...
  'youtube', 'google',
  'obs', 'streamlabs', 'bandicam', 'camtasia', 'sharex', 'snipping tool',
  'screenpresso', 'screencast', 'lightshot', 'flashback',
  'zoom', 'teams', 'meet', 'webex', 'skype', 'slack',
...
class ProctorService with WidgetsBindingObserver {
  ...
  bool _handleKey(KeyEvent event) {
    ...
    if (ctrl && (key == LogicalKeyboardKey.keyC || key == LogicalKeyboardKey.keyV)) {
      onIncident(DesktopEvent(
        title: 'Portapapeles',
        details: 'Atajo Ctrl+${key == LogicalKeyboardKey.keyC ? 'C' : 'V'} en el examen.',
        severity: 'leve',
        at: DateTime.now(),
      ));
    }
  ...
  Future<void> _inspectWindow() async {
    ...
    onIncident(DesktopEvent(
      ...
      details:
          '[${severity.toUpperCase()}] Aplicacion: ${snapshot.processName} | Ventana: ${snapshot.windowTitle}',
      severity: severity,
      at: DateTime.now(),
    ));
  }
  ...
  Future<void> _scanProcesses() async {
    ...
    for (final process in processes) {
      final isRecorder = _mediaKeywords.any((keyword) =>
  ...
  bool _isSelf(ActiveWindowSnapshot snapshot) {
    final text =
        '${snapshot.processName} ${snapshot.windowTitle}'.toLowerCase();
